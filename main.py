    ...
    try:
        ...
        def callback(call):
            if call.data == "Digital":
                ...
                logging.info("Reject request", extra={"user": bot.get_chat(id_of_user).username})
                ...
                if isDigital:
                    ...
                    with open(result_filename, "rb") as f:
                        bot.send_document(PRINTER_CHAT_ID, f,
                                          caption=id_of_user + " " + call.message.text.split('\n')[0].split(' ')[1]
                ...
                bot.send_message(call.message.caption.split('\n')[0], f"Cправка для " +
                                 ...
                                 f" готова, можно забирать в {PLACE}")
                logging.info("Send to user", extra={"user": bot.get_chat(call.message.caption.split('\n')[0]).username})
